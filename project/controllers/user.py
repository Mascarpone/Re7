...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            flash(u'Vous vous etes enregistre avec le nom d utilisateur {0}. Veuillez vous connecter.'.format(login))
            return redirect(url_for('login'))
        else:
            flash(u'Le nom d utilisateur {0} est deja utilise. Veuillez choisir un autre nom.'.format(login))
